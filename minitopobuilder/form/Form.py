...
class Form( object ):
    ...
    def prepare_coords(self):
        ...
        for x in range(len(self.sslist) - 1):
            ...
            ssregion.append([i-len(self.sslist[x].sequence),i])
            ...
            if self.l_linkers != None and self.l_linkers[x+1] != "x":
                ...
                # d = int(math.ceil(d / 3.))
                # d = random.randint(d+3,d+6) # 2 ,4
                d = int(math.ceil(d/2.5))
                if d>4:
                    d = random.randint(d-1,d+2)
                else:
                    d = random.randint(d,d+2)
                looplenlist.append(d)
            i += d
            for yy in range(d):
                self.seq_str.append(("G", "C", "X"))
            self.inits.append(i)
        i +=len(self.sslist[-1].sequence)
        ssregion.append([i-len(self.sslist[-1].sequence),i])
