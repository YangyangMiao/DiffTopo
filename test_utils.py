...
# test data loader
from data import *
data_path = './12SSE_node_distmat_edgeattr_onehot_len.pt'
dataloader = get_dataloader(data_path,2,'cpu',shuffle=True)
for data in dataloader:
    tmpdata = data
    print(data['edgeattr'][0].shape)
    break

p = argparse.ArgumentParser(description='E3Diffusion')
p.add_argument('--config', type=argparse.FileType(mode='r'), default='./test.yml')
...
p.add_argument('--normalize_factors', type=eval, default=[1, 4], help='normalize factors for [x, categorical]')
...
p.add_argument('--wandb_entity', type=str, default='lpdi', help='Entity (project) name')
p.add_argument('--enable_progress_bar', action='store_true', default=False, help='Progress bar when training')
p.add_argument('--center_of_mass', type=str, default='interface', help='Where to center the data: interface | motif')
