...
def remove_partial_mean_with_mask(x, node_mask, center_of_mass_mask,
    ...
    if has_batch_dim:
        ...
        x_masked = x * center_of_mass_mask
        N = scatter_sum(center_of_mass_mask, node_mask, dim=0)
        mean = scatter_sum(x_masked, node_mask, dim=0) / N
        x = x - mean[node_mask]
    return x
...
def recover_from_oom(out_type: str = 'data', static: bool = True):
    """
    Use this as a decorator for an operation acting on an entire batch.
    If an out-of-memory error occurs, 'vectorized_fn' will be applied to each
    batch item in a loop.
    Functions must accept a batch_mask as the first input argument followed by a
    number of batched data items, where the first dimension corresponds to the
    indices in batch_mask.
    Args:
        out_type: either 'data' or 'index', default: 'data'
            If 'data', the outputs for each batch are stacked with dim=0.
            If 'index', the outputs are mapped back to global indices and
            stacked with dim=1.
        static: set to False if 'self' argument is passed to the decorated
            function
    """
    assert out_type in {'data', 'index'}

    def decorator(vectorized_fn: Callable):
        def wrapper(*args):
            try:
                out = vectorized_fn(*args)
            except RuntimeError as e:
                if 'out of memory' in str(e):
                    warnings.warn(
                        f"Vectorized computation '{vectorized_fn.__name__}' "
                        f"ran out of memory")
                    torch.cuda.empty_cache()

                    if static:
                        batch_mask, batched_data = args[0], args[1:]
                    else:
                        (self, batch_mask), batched_data = args[:2], args[2:]

                    out_list = []
                    for i in torch.unique(batch_mask):
                        inds = torch.where(batch_mask == i)[0]
                        data_i = [x[inds] for x in batched_data]
                        # batch_mask_i = batch_mask[inds]
                        batch_mask_i = torch.zeros_like(batch_mask[inds])
                        if static:
                            out_i = vectorized_fn(batch_mask_i, *data_i)
                        else:
                            out_i = vectorized_fn(self, batch_mask_i, *data_i)

                        if out_type == 'index':
                            out_i = inds[out_i]
                        out_list.append(out_i)

                    out = torch.cat(out_list, dim=0 if out_type == 'data' else 1)
